...
class CopiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create copy" do
    assert_difference("Copy.count") do
      post copies_url, params: { copy: { name: @copy.name, position: @copy.position, visible: @copy.visible } }
    end
  ...
  test "should update copy" do
    patch copy_url(@copy), params: { copy: { name: @copy.name, position: @copy.position, visible: @copy.visible } }
